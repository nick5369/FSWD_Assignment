...
function ProfileModal({ modalOpened, setModalOpened, data }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (profileImage) {
      try {
        const base64ProfileImage = await convertToBase64(profileImage);
        UserData.profilePicture = base64ProfileImage;  // Store Base64 string
      } catch (error) {
        console.log("Error converting profile image to Base64:", error);
      }
    }
  
    if (coverImage) {
      try {
        const base64CoverImage = await convertToBase64(coverImage);
        UserData.coverPicture = base64CoverImage;  // Store Base64 string
      } catch (error) {
        console.log("Error converting cover image to Base64:", error);
      }
    }
